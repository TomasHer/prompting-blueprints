...
## Unreleased
- Add AI Job Search use-case tutorial (`07-use-cases-and-research/ai-job-search-claude-code.md`) grounded in the open-source **`MadsLorentzen/ai-job-search`** framework (MIT) — an end-to-end job-application agent built on **Claude Code**. Covers the fork-and-fill project shape (seven numbered profile "skill" files that separate user data from automation), the slash-command workflow (`/setup`, `/scrape`, `/apply`, `/expand`, `/upskill`, `/add-template`, `/add-portal`), and three reusable prompt-engineering patterns: the **drafter–reviewer split** (a second Claude agent spawned with a *fresh context* researches the company and critiques the drafts), the **visual PDF-verification loop** (compile → inspect the rendered PDF → apply targeted layout fixes until clean), and **relevance-weighted CV trimming**; cross-links the related LangChain researcher ↔ critique pattern, wires the page into the MkDocs nav and `structure.txt`, and registers the repo in `external-sources.md` / `source-index.md`.
