...
## Unreleased
- Add Agent Memory for .NET tutorial (`02-ai-agents/03-context-and-memory/agent-memory-dotnet-tutorial.md`) covering `joslat/agent-memory-dotnet` — a persistent, graph-native memory engine for AI agents backed by Neo4j, a from-scratch .NET reimplementation (not a port) inspired by Neo4j Agent Memory and verified against its compatibility kit (178/178 Bronze/Silver/Gold). Explains the three interconnected memory layers (short-term conversations/sessions, long-term entities/facts/preferences/relationships/provenance/temporal state, reasoning traces/tool calls/task patterns), the four retrieval strategies (vector, full-text, hybrid, graph traversal + optional GraphRAG), the persistence-layer governance model (ownership via `owner_id`/`MemoryScope`, provenance via `EXTRACTED_FROM`/`EXTRACTED_BY` edges, bitemporal valid-time vs. transaction-time, audit logs, soft-invalidation, tenant isolation), the "Mnemonic Core" identity, ecosystem adapters (Microsoft Agent Framework, Semantic Kernel, MCP, Microsoft.Extensions.AI, OpenTelemetry, Neo4j GDS), and a runnable quick start (Docker Neo4j, DI registration, `ISchemaBootstrapper`, `IMemoryService.AddMessageAsync`/`ExtractAndPersistAsync`/`RecallAsync`/`RecallAsOfAsync`), plus a transcript of José Luis Latorre's release announcement. Wire it into the MkDocs nav next to the existing AgentMemory (Python/SQLite) tutorial and register the `joslat/agent-memory-dotnet` and upstream `neo4j-labs/agent-memory` repos in `external-sources.md` / `source-index.md`.
