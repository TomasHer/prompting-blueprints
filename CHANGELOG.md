...
## Unreleased
- Add a "maturity scale: from Prompter to System Architect" section to the harness-engineering primer (`02-ai-agents/01-foundations/prompt-context-harness-engineering.md`), mapping the four engineering layers to the Prompter → Operator → Loop Engineer → System Architect role progression (with the System Architect rung linking to `agent-fleet-governance.md`), and register Rakesh Gohel's *"The difference between a Prompter and a Loop Engineer"* infographic in the references. Name the **curator** primitive explicitly in `context-engineering.md` ("keep what matters; drop what does not") so the page matches the curate-inputs-into-the-window framing.
