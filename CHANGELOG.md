...
## Unreleased
- Add the Kaggle/Google whitepaper *The New SDLC with Vibe Coding* (Addy Osmani, Shubham Saboo, Sokratis Kartakis) to the guides library (`04-guides/overview.md`) and register it in `source-index.md` / `external-sources.md`; digest its *"Harness Engineering: What surrounds the model"* section into the harness-engineering primer (`02-ai-agents/01-foundations/prompt-context-harness-engineering.md`) with a new section covering the full component inventory (prompts, tools and the tool-call lifecycle, context policies/compaction, **hooks** as the deterministic enforcement layer, sandboxes, subagents with condensed summaries, feedback/recovery paths, the `AGENTS.md` rolling rulebook plus session-log memory, progressively disclosed skills, and outside-the-model verification), the ratchet principle, and the work-backwards design discipline.
