...
| [Hands-on Lab — Agentic RAG: AI that Decides — Retrieval to Autonomy](#lab-agentic-rag) | Toufique Hasan & Ayman Khan, GPT-Lab / Tampere University |
| [Hands-on Workshop — Agentic Workflows and Artifact Representation](#workshop-agentic-workflows) | François Christophe & Konsta Kalliokoski, University of Jyväskylä |
| [Spotlight Talk — Engineering Trustworthy AI in Europe: Compliance & Responsibility](#spotlight-salla) | Salla Westerstrand, GPT-Lab / Tampere University |
...
<a id="lab-agentic-rag"></a>

### 🛠️ Hands-on Lab — Agentic RAG: AI that Decides — Retrieval to Autonomy by Toufique Hasan & Ayman Khan, GPT-Lab / Tampere University

<img src="../assets/conferences/gaise-2026/day3/04-ayman/IMG_8095.JPG" alt="Agentic RAG: AI that Decides — Retrieval to Autonomy — hands-on academic lab by Toufique Hasan & Ayman Khan, GPT-Lab / Tampere University; the opening 'Evolution from RAG to Agentic RAG' slide">

Day 3's hands-on academic track took the conference's governance theme and made it something you could run on your own laptop. Toufique Hasan and Ayman Khan of GPT-Lab walked the room from **classic RAG** — a fixed retrieve-then-generate pipeline — to **Agentic RAG**, where the model plans, calls tools, critiques its own evidence, and synthesizes an answer inside a loop, all under explicit governance. The worked example was deliberately un-flashy: a **Bio AI Fishery Assistant** for Lake Pyhäjärvi that answers catch-statistics, economic, and water-quality questions — chosen precisely because the answers must be *exactly right*, not merely plausible-sounding. The throughline, repeated like a mantra: **the LLM is not doing the calculations** — agents reason about *what* should be computed, but trusted, deterministic Python tools do the computing, and autonomy is something you dial in on purpose rather than turn loose. Materials and both reference implementations: [GPT-Laboratory/GAISE_AgenticRAG](https://github.com/GPT-Laboratory/GAISE_AgenticRAG).

**Interesting observations**

- **From classic RAG to Agentic RAG — retrieval becomes a loop, not a pipeline.** Classic RAG is a straight line: *User query → Embed → Retrieve Top-K → Rerank/Context → LLM → Answer.* That is plenty when the job is fetching text passages. The fishery assistant isn't only fetching text — it has to compute yearly catch totals, rank species, estimate economic value, correlate catches with water quality, and synthesize the result — so the shape changes to *Plan → Retrieve/Analyze → Use Tools → Critique Evidence → Synthesize*, wrapped around an **iterative reasoning & retrieval** core that can fan out to multiple agents, tool/API calls, web search, long-term memory, a human-approval checkpoint, and an audit log (pictured above). It's the same "the loop is the intelligence" lesson Jussi Rasku drew in his [tool-building lab](#lab-jussi-rasku) — generate → act → observe → critique → revise — here pointed at retrieval ([loop engineering](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/01-foundations/loop-engineering.md)).

- **The use case is deterministic on purpose — "the LLM is not doing the calculations."** The assistant runs over cleaned CSVs of Lake Pyhäjärvi data — catch statistics (12 species, 2010–2024), Luke economic price statistics (855 rows reaching back to 1980), and water-quality metrics — and the single most-repeated line of the lab was a guardrail: *"the agents reason about what should be done, but trusted Python tools compute the fishery, economic, and environmental results."* The processed CSVs are the knowledge base; the agents never invent a number, they call a tool that reads the file and computes it with pandas. Choosing a domain where a wrong number is *obviously* wrong is exactly what makes the governance argument land.

- **The custom-tools layer is where autonomy is actually governed.** Every tool follows one shape — *Tool = name + description + `_run()` + analytics function* — so the model only ever sees a name and a description, and when it picks a tool the `_run()` method hands off to deterministic Python. The tools cluster into catch/species analytics, economic valuation (the entire economic model is just `estimated_value_eur = catch_kg × price_eur_per_kg`), and environmental correlation. Because tool names are explicit, access is limited per agent, outputs are structured, and results carry source fields, the tool layer *is* the governance layer — the same "wrap deterministic capability behind a typed, described tool" idea that [MCP](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/04-protocols/mcp-guide.md) formalizes and that gives an agent its "hands" in the Day 1 [harness demo](#demo-harnesses).
...
<br>**② Guided Reasoning Loop** — human as *Collaborator* (retrieve → compare → critique → answer).
<br>**③ Supervised Tool Use** — human as *Approver* (agent → call API/tool → approve or reject).
<br>**④ Delegated Workflow Agent** — human as *Consultant/Approver* (plan → retrieve → reason → act → verify, backed by memory, tools, monitoring, and rollback).
<br>**⑤ High-Autonomy Multi-Domain Agent** — human as *Observer* (web, data, and APIs flowing through the agent to docs, analytics, and external systems inside a sandbox/policy boundary).
<br>This project sits around rungs **③–④**, and a vertical arrow up the ladder reads *more autonomy = stronger governance needed* — the same approve-or-automate dial that ran through Alex Polyakov's [human-in-the-loop keynote](#keynote-alex-polyakov) and, that very morning, Tommi Mikkonen's [four-stage handover model](#keynote-tommi-mikkonen), here drawn as five rungs.

<img src="../assets/conferences/gaise-2026/day3/04-ayman/IMG_8103.JPG" alt="Autonomy and Governance in Agentic RAG — a five-rung autonomy ladder (Assistive Retrieval, Guided Reasoning Loop, Supervised Tool Use, Delegated Workflow Agent, High-Autonomy Multi-Domain Agent) with shifting human roles, alongside a Governance Stack of principles, risk-management frameworks, and legal obligations">

- **The governance stack names the actual standards — and the system stays "bounded."** Next to the ladder sat a concrete **governance stack** rather than hand-waving: *Principles* — OECD, UNESCO; *Risk Management* — NIST AI RMF, ISO/IEC 42001, ISO/IEC 23894; *Legal Obligations* — EU AI Act; *Operational Controls* — task checks, constrained actions, human approvals, monitoring, audit logs, and incident reporting, with any unsafe or out-of-policy tool use *"blocked & logged."* The bottom line, stated plainly: *"the system is not fully autonomous — it is a controlled workflow with bounded autonomy"* — a fitting Responsible-AI-Era counterpart to Mika Torhola's regulated [traceability pipeline](#demo-spexant) and Esa Karjalainen's [agent-security framing](#lab-esa-karjalainen), and a concrete answer to Meeri Haataja's call to move governance [from forms to events](#keynote-meeri-haataja).

- **Two tracks — proprietary and fully-local open-source.** Groups could build on either of two reference stacks: **OpenAI + CrewAI** (the agent architecture above) or a **fully local Ollama + LangGraph** system that runs with no API keys, no cloud calls, and $0 recurring cost — a **Qwen 2.5 14B** model via Ollama, `multilingual-e5` embeddings, and a local Chroma vector store, orchestrated as a LangGraph state machine that adds a **corrective-RAG** step (it grades the retrieved passages and, if they're weak, reformulates the query and retrieves once more before answering). A nice practitioner's detail: they deliberately collapse ~24 fine-grained analytics functions into **11 tools**, because a local 14B model gets measurably worse at *picking* the right tool once it faces more than ~10–12 look-alike options — fewer, broader tools beat more, finer ones ([open models](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/01-foundations/open-models.md)).

- **Hands-on: build one yourself — Build → Test → Compare → Reflect → Learn.** The lab then handed over the keyboard: six groups, split across the two tracks, each picking a domain (agriculture, news, or medical) and wiring up a minimal Agentic RAG prototype — a user question, a router/planner agent, a retrieval component, one or more domain tools, an optional critic/evaluator, and a final answer generator. The framing was explicitly anti-perfectionist — *"the goal is not to build a perfect production system"* but to feel how retrieval connects to reasoning and how workflow design controls autonomy — and each group closed with a Mentimeter retrospective on what worked, what failed, and what they'd improve. A build-it-yourself coda to a governance-heavy day: the model is interchangeable, but the tools, scoping, and workflow wrapped around it are what make an agent trustworthy ([AI agents overview](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/01-foundations/ai-agents-overview.md)).

<a id="workshop-agentic-workflows"></a>

### 🛠️ Hands-on Workshop — Agentic Workflows and Artifact Representation by François Christophe & Konsta Kalliokoski, University of Jyväskylä

Closing out Day 3's hands-on track, François Christophe and Konsta Kalliokoski of the **University of Jyväskylä** — Tommi Mikkonen's home institution — turned the morning's governance theme into a practical exercise aimed not at *building* agents but at *representing* what they do and *evaluating* what they produce. The premise: once agents generate artifacts faster than people can read them, the bottleneck moves off the model and onto the humans around it — how the workflow is organized, how agents are given context, and how the cognitive load of reviewing the output is kept manageable. So the room did the reviewing: agent-produced artifacts were put up for interactive evaluation by the audience, while the session walked through different ways to organize multi-agent workflows, how to feed agents context, and how to make the resulting artifacts legible enough to judge. The throughline: an agentic system's value isn't model IQ — it's coordination, feedback, ownership, and the human who decides when the loop stops.

**Interesting observations**

- **"Less about model IQ, more about coordination, feedback, and ownership."** The foundational contrast set the frame. A **normal workflow** is a straight line — *Input → Execute once → Output* — linear, with no replan and no escalation: a single pass with limited checks and weak traceability. An **agentic workflow** wraps that in a loop — a *Goal* handed to an *Orchestrator* that fans work out to *Research, Code, and Review*, then *Validate*, then a decision gate (*passed checks?*) that either ships to *Delivery* or escalates to *Human review*, with the review result fed back to the orchestrator. The labels underneath name the delta: orchestrated, feedback loop, escalation path, shared state, replan, accountable output. It's the same "the loop is the intelligence" lesson Jussi Rasku drew in his [tool-building lab](#lab-jussi-rasku) and the harness framing of the [Day 1 harness demo](#demo-harnesses) — here rendered as a side-by-side that makes the agentic difference legible at a glance ([loop engineering](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/01-foundations/loop-engineering.md)).

- **Three multi-agent team patterns for placing a human among agents.** When a human collaborates with a group of agents, the structure usually settles into one of three shapes. **Flat Teams** — the human and specialized agents sit on the same hierarchy level, communicating openly in a shared canvas or message stream. **Hierarchical Systems** — a supervisor agent orchestrates subagents as specialized tools, shielding the human from the *micro-coordination tax* behind a single, consolidated interface. **Group Discussion / Swarm** — multiple agents hold a structured, asynchronous conversation, attacking a problem from different perspectives and emitting an accumulated log for final human validation. Each trades coordination overhead against human visibility differently, and together they are the individual-scale counterpart to the org-level structures Day 2's [agentic-organizations panel](#panel-agentic-organizations) wrestled with — and to the agents-that-breed-agents "multiverse" Pekka Abrahamsson [demoed](#talk-pekka-abrahamsson) ([Agent-to-Agent (A2A) protocol](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/04-protocols/a2a-protocol-guide.md), [AI agents overview](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/01-foundations/ai-agents-overview.md)).

- **"Do interactions really solve issues?" — an honest *not by themselves*.** The workshop pushed back on the easy assumption that more agent-to-agent interaction automatically yields better answers. Three caveats: *loads of documents to review* isn't really the problem (the software stays the developers' responsibility either way); interaction *does not guarantee quality*; and it *does not guarantee a limited number of looping iterations*. The conclusion is where the Responsible-AI-Era theme lands squarely — these are exactly the criteria where human expertise and critical thinking are still needed, and **in the end the human sets the stopping condition for the iterations.** It's the same hand-on-the-wheel point as Alex Polyakov's [human-in-the-loop keynote](#keynote-alex-polyakov) and Tommi Mikkonen's [four-stage handover model](#keynote-tommi-mikkonen) from that same morning, and the human-judgment twin of the per-agent iteration caps (`max_iterations`) the [Agentic RAG lab](#lab-agentic-rag) used to bound autonomy.

- **Artifact representation is a cognitive-load problem — and the audience felt it firsthand.** The session's real subject was right in its title: not only how agents *work*, but how their *artifacts are represented* so a human can evaluate them without drowning. Handing the audience agent-produced artifacts to grade turned that into something experiential — you feel immediately how structure, supplied context, and traceability decide whether review is tractable or hopeless. It's a hands-on take on Markus Borg's warning about [code bloat, cognitive load, and treating understanding as a KPI](#keynote-markus-borg), and on the [context-engineering](https://github.com/TomasHer/prompting-blueprints/blob/main/02-ai-agents/03-context-and-memory/context-engineering.md) case that what makes agent output usable is good context and representation, not a bigger model. A fitting hands-on coda to the Responsible AI Era: the work that doesn't disappear is the human judgment about *when an artifact is good enough and the loop can stop.*

<a id="spotlight-salla"></a>

### 🎓 Spotlight Talk — Engineering Trustworthy AI in Europe: Compliance & Responsibility by Salla Westerstrand, GPT-Lab / Tampere University

<img src="../assets/conferences/gaise-2026/day3/06-salla/IMG_8105.JPG" alt="Engineering Trustworthy AI in Europe: Compliance & Responsibility — spotlight talk by Salla Westerstrand, GPT-Lab / Tampere University">

In the day's final spotlight before the conference wrap-up, Salla Westerstrand drew a sharp line between two words the industry tends to use interchangeably: **compliance** and **responsibility**. Her argument: the EU AI Act is the *floor*, not the finish line. Meeting it makes your AI legal; it does not make it trustworthy, and it certainly doesn't make it valuable. Borrowing the language of antifragility from Taleb and Spitz, she reframed the regulatory moment not as a burden to resist but as a disruption to *grow from* — the difference between an organization that merely survives the shock of AI-plus-regulation and one that compounds value because of it. The throughline: compliance is the minimum every European AI system owes; responsibility is the ambition that turns the same constraints into an antifragile advantage.

**Interesting observations**

- **"The EU AI Act is the minimum."** The talk's opening move — Section 01, literally titled *The Minimum* — reframed Europe's flagship regulation as a baseline, not a goalpost. Compliance clears the legal bar; it says nothing about whether a system is fair, useful, or worth building — exactly the gap between *legal* and *trustworthy* that the rest of the talk set out to close. It's the regulatory counterpart to Tommi Mikkonen's morning [From Vibes to Engineering](#keynote-tommi-mikkonen) thesis: clearing the bar is the easy part, the engineering is everything above it.
